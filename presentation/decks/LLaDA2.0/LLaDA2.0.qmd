...
- (Discrete) Diffusion Large Language Models (dLLM) dLLM は自己回帰モデル (ARM) に対する有望な代替パラダイムだが、実用規模への拡張が未実証だった
...
- **AR 初期化からの変換**: 既存研究はごく小規模（数百M〜1B）に留まる
  - 本研究は変換の対象を **16B・100B** まで拡張する初の試み
- **dLLM の事後学習**: SFT 相当の手法は一部提案されているが、DPO 等の preference alignment は開拓の余地あり

---

# 新規性 (3)

既存の ARM チェックポイントを安定的に dLLM へ変換するための 3 段階の学習スキーム (WSD) を確立
加えて以下の技術的貢献が組み合わさっている。

- **WSD (Warmup-Stable-Decay) Strategy** — ブロックサイズを段階的に拡大・縮小し、ARM から dLLM へ知識を引き継ぐ
...
# アイデア：WSD Strategy (4.1)

ARM チェックポイントは「ブロックサイズ=1 の Block Diffusion LM」と解釈できる。
ここからブロックサイズを段階的に拡大・縮小することで ARM → dLLM の変換を実現する。
...
**Block Diffusion Language Models (BDLM) の損失関数**
...
Stable フェーズでは $K=1$（全系列が 1 ブロック）となり、標準的な MDLM の目的関数に帰着
...
- それらのパラメータを算術平均でマージ
- 事後学習への初期化として、単一ベストチェックポイントやEMAより安定した性能を示す
...
$$
\mathcal{L}_{DPO} =  -\mathbb{E}\left[ \log \sigma(\beta\left[ \Delta \mathbf{B}(\mathbf{x}_\omega | \mathbf{c}) - \Delta \mathbf{B}(\mathbf{x}_l | \mathbf{c})\right])\right]
$$

- $\Delta \mathbf{B}(\mathbf{x}|\mathbf{c}) = B_{BDLM}(\theta, \mathbf{x}|\mathbf{c}) - B_{BDLM}(\theta_{ref}, \mathbf{x}|\mathbf{c})$
- $\beta$: ハイパラ(0.1)
...
![LLaDA2.0-mini のベンチマーク](figures/table1_llada2_mini_benchmark_performance.png)
...
![LLaDA2.0-flash の主要ベンチマーク結果](figures/fig1_llada2_flash_main_results.png)

---

# 実験結果：LLaDA2.0-flash 詳細表 (6.2)

![LLaDA2.0-flash の詳細ベンチマーク](figures/table2_llada2_flash_benchmark_performance.png)

---

# 実験結果：推論速度 (6.3)

8×H20 GPU (TP8) 上での throughput の比較
...
- 100B を超えるスケールへの拡張
- 拡散モデル向けの強化学習手法の確立 (LLaDA2.1)・テスト時推論（test-time thinking）
- 64k 以上のコンテキスト長拡張
...
- LLaDA 2.0 は **ARM チェックポイントから dLLM へ変換する WSD Strategy**を確立し、dLLMを初めて 100B 規模へ到達させた
