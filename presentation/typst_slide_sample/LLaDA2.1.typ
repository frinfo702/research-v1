...
// ── モチベーション ────────────────────────────────────────────
= Exposure bias が並列デコードの失敗を固定化する ref #2
...
// ── 先行研究 ─────────────────────────────────────────────────
= 既存手法には「確定トークンの書き換え」を学習する目的がない ref #1
...
// ── 新規性 ───────────────────────────────────────────────────
= LLaDA2.1 はスケール拡張ではなく editable decoding を導入する ref #1
...
// ── アイデア ─────────────────────────────────────────────────
= Token editing により並列生成しながら自己訂正できる ref #2
...
// ── 提案手法：訓練 ────────────────────────────────────────────
= Mixture 学習で 1 つのモデルに drafting と editing を両立させる ref #3.1
...
// ── 提案手法：推論と RL ───────────────────────────────────────
= S/Q mode と EBPO で推論設定と RL を同時に拡張する ref #3.2, #4.3
...
// ── 実験設定 ─────────────────────────────────────────────────
= 評価は 33 タスクで品質と throughput を同時に測る ref #5
...
// ── 実験結果：Flash ───────────────────────────────────────────
= Q Mode は Flash で LLaDA2.0 を品質と TPF の両方で上回る ref #5
...
// ── 実験結果：Mini ────────────────────────────────────────────
= Q Mode は Mini でも品質を改善しつつ TPF を伸ばす ref #5
...
// ── 実験結果：スループット ────────────────────────────────────
= 量子化した S Mode は Flash で最大 891.74 TPS に達する ref #5
...
// ── アブレーション ────────────────────────────────────────────
= MBE と editing 学習は特に推論・コード性能に効く ref #5
...
// ── 制限 ─────────────────────────────────────────────────────
= Editing で改善しても stuttering と KV cache 制約は残る ref #6
...
// ── まとめ ───────────────────────────────────────────────────
= Token editing により速度と品質のトレードオフは「固定」から「調整可能」になる ref #6
